...
export const GameCanvas: React.FC<GameCanvasProps> = ({
  ...
  const [announcementTimer, setAnnouncementTimer] = useState(180);
  ...
  const startWave = useCallback((sectorIdx: number, waveNum: number) => {
    ...
    if (isBoss) {
      ...
      setWaveAnnouncement(`WARNING: BOSS APPROACHING!`);
      setAnnouncementTimer(200);
      ...
      setWaveAnnouncement(`LEVEL ${sectorIdx + 1} - WAVE ${waveNum}: ENEMIES INCOMING`);
      setAnnouncementTimer(160);
  ...
  const advanceSector = useCallback(() => {
    ...
    setWaveAnnouncement(`LEVEL ${sectorIndex + 1} CLEARED! WARPING TO NEXT LEVEL`);
    setAnnouncementTimer(220);
    ...
    setTimeout(() => {
      setSectorIndex(prev => {
        ...
        if (nextIdx >= SECTORS.length) {
          ...
          setWaveAnnouncement(`VICTORY! ALL LEVELS CLEARED`);
          return prev;
        } else {
          setCurrentWave(1);
  ...
  useEffect(() => {
    ...
    const handleBlur = () => {
      // Pause game automatically when window loses focus (only if actively in game)
      if (!isMainMenu) {
        setIsPaused(true);
      }
    };
  ...
  }, [toggleSettings, isMainMenu, waveState, startGame, restartGame, onToggleMute]);
  ...
  useGameLoop((deltaTime: number) => {
    ...
    // Decay Screen Shake
    if (screenShake > 0) {
      setScreenShake(prev => Math.max(0, prev - 0.8));
    }
    ...
    shipsRef.current.forEach(ship => {
      ...
      if (isP1) {
        isThrusting = keys.current.has('KeyW');
        isBraking = keys.current.has('KeyS');
        isRotatingLeft = keys.current.has('KeyA');
        isRotatingRight = keys.current.has('KeyD');
        isFiring = keys.current.has('Space');
      } else if (isDrone) {
        ...
        // 2-Player Local
        isThrusting = keys.current.has('ArrowUp');
        isBraking = keys.current.has('ArrowDown');
        isRotatingLeft = keys.current.has('ArrowLeft');
        isRotatingRight = keys.current.has('ArrowRight');
        isFiring = keys.current.has('Enter');
      }
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (announcementTimer > 0) {
        ...
        ctx.shadowBlur = 15;
        ctx.shadowColor = '#00e5ff';
        ctx.fillText(waveAnnouncement, GAME_WIDTH / 2, GAME_HEIGHT / 2 + 6);
        ctx.restore();
      }
  ...
  }, [currentSector, screenShake, announcementTimer, waveAnnouncement, waveState, warpProgress]);
